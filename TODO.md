...
1. Added `(2,3)` mixed-regime OPE extraction aliases in `Correlators.lean`
   using consistent `left/right` naming (left = `(b,c)`, right = `(c,b)`):
   - formal-distribution layer:
...
1. Added `(1,2)` mixed-regime OPE extraction aliases in `Correlators.lean`
   using explicit orientation naming (`ab/ba`):
   - formal-distribution layer:
     `threePointCommutator12_eq_opeCoefficient_of_ge_ab_lt_ba`,
     `threePointAnticommutator12_eq_opeCoefficient_of_ge_ab_lt_ba`,
     `threePointCommutator12_eq_neg_opeCoefficient_of_lt_ab_ge_ba`,
     `threePointAnticommutator12_eq_opeCoefficient_of_lt_ab_ge_ba`
2. Added matching state-level alias wrappers:
   - `threePointStateCommutator12_eq_opeCoefficient_of_ge_ab_lt_ba`
   - `threePointStateAnticommutator12_eq_opeCoefficient_of_ge_ab_lt_ba`
   - `threePointStateCommutator12_eq_neg_opeCoefficient_of_lt_ab_ge_ba`
   - `threePointStateAnticommutator12_eq_opeCoefficient_of_lt_ab_ge_ba`
3. Post-expansion check:
   - `lake build StringAlgebra.VOA` passes
   - theorem-level `sorry` count in `StringAlgebra/VOA/*.lean` remains `0`
...
1. Added `(2,3)` mixed-regime OPE extraction aliases in `Correlators.lean`
   using explicit orientation naming (`bc/cb`):
   - formal-distribution layer:
     `threePointCommutator23_eq_opeCoefficient_of_ge_bc_lt_cb`,
     `threePointAnticommutator23_eq_opeCoefficient_of_ge_bc_lt_cb`,
     `threePointCommutator23_eq_neg_opeCoefficient_of_lt_bc_ge_cb`,
     `threePointAnticommutator23_eq_opeCoefficient_of_lt_bc_ge_cb`
2. Added matching state-level alias wrappers:
   - `threePointStateCommutator23_eq_opeCoefficient_of_ge_bc_lt_cb`
   - `threePointStateAnticommutator23_eq_opeCoefficient_of_ge_bc_lt_cb`
   - `threePointStateCommutator23_eq_neg_opeCoefficient_of_lt_bc_ge_cb`
   - `threePointStateAnticommutator23_eq_opeCoefficient_of_lt_bc_ge_cb`
3. Post-expansion check:
   - `lake build StringAlgebra.VOA` passes
   - theorem-level `sorry` count in `StringAlgebra/VOA/*.lean` remains `0`
...
1. Refactored `Modules.lean` into a dedicated subfolder layout:
   - `StringAlgebra/VOA/Modules/Core.lean`
   - `StringAlgebra/VOA/Modules/Intertwining.lean`
   - `StringAlgebra/VOA/Modules/Fusion.lean`
   - `StringAlgebra/VOA/Modules/Advanced.lean`
2. Converted `StringAlgebra/VOA/Modules.lean` into a lightweight facade module
   that imports and re-exports the split module-theory submodules, preserving
   the existing root import path used by `StringAlgebra/VOA.lean`.
3. Post-refactor check:
   - `lake build StringAlgebra.VOA` passes
   - theorem-level `sorry` count in `StringAlgebra/VOA/*.lean` remains `0`
